...
exec { 'Download Varnish Source':
  command => 'curl -sSL https://repo.varnish-cache.org/source/varnish-4.0.1.tar.gz | tar -C /root -zx',
  creates => '/root/varnish-4.0.1',
...
exec { 'Configure Varnish Source':
  cwd     => '/root/varnish-4.0.1',
  command => '/root/varnish-4.0.1/configure --prefix=/usr --enable-debugging-symbols',
  creates => '/root/varnish-4.0.1/config.status',
...
exec { 'Make Varnish Source':
  cwd     => '/root/varnish-4.0.1',
  command => 'make',
  creates => '/root/varnish-4.0.1/bin/varnishd/varnishd',
...
exec { 'Git Clone Varnish Agent 2':
  command => 'git clone https://github.com/varnish/vagent2.git /root/vagent2 && cd /root/vagent2 && git checkout 4.0',
